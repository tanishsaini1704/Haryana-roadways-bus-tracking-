...
# ================== CONFIGURATION ==================
# Make sure your OpenRouteService API key is here
...
def load_cache_from_disk():
    if os.path.exists(CACHE_FILE):
        with open(CACHE_FILE, 'r') as f:
            ...
            return json.load(f)
    print("No cache file found. A new one will be created on the first run.")
    return {}
...
ROUTE_WAYPOINTS = {
    # User's Special Route (Preserved)
...
for key, waypoints in ROUTE_WAYPOINTS.items():
    final_waypoints[key] = waypoints
    start, end = key.split('-')
    reverse_key = f"{end}-{start}"
    if reverse_key not in ROUTE_WAYPOINTS:
        final_waypoints[reverse_key] = waypoints[::-1]
ROUTE_WAYPOINTS = final_waypoints


@app.route('/get_all_routes')
def get_all_routes():
    return jsonify(list(ROUTE_WAYPOINTS.keys()))

@app.route('/get_route')
def get_route():
    start_city = request.args.get('start', '').lower()
    end_city = request.args.get('end', '').lower()
    cache_key = f"{start_city}-{end_city}"

    if cache_key in route_cache:
        print(f"Route '{cache_key}' found in cache. Returning instantly.")
    ...
    try:
        ...
        for i, city in enumerate(all_cities):
            ...
            if i == 0 or i == len(all_cities) - 1:
                coords.append(BUS_STAND_COORDINATES[city])
            else:
                coords.append(HIGHWAY_WAYPOINT_COORDINATES.get(city, BUS_STAND_COORDINATES[city]))
        
        print(f"Fetching '{cache_key}' from API and saving to cache...")
        directions_result = ors_client.directions(
            coordinates=coords, profile='driving-hgv', format='geojson', instructions=False
        )
        summary = directions_result['features'][0]['properties']['summary']
        
        points_lon_lat = directions_result['features'][0]['geometry']['coordinates']
        route_for_frontend = [{"lat": lat, "lng": lon} for lon, lat in points_lon_lat]

        # BUG FIX: Replaced the complex and faulty list comprehension with this safer loop.
        stops_info = []
        for i, city_name in enumerate(all_cities):
            if city_name not in BUS_STAND_COORDINATES: continue
            
            lon, lat = (0, 0)
            if i == 0 or i == len(all_cities) - 1:
                lon, lat = BUS_STAND_COORDINATES[city_name]
            else:
                # Use .get() for safety, providing a fallback to the bus stand coordinate
                lon, lat = HIGHWAY_WAYPOINT_COORDINATES.get(city_name, BUS_STAND_COORDINATES[city_name])
            
            stops_info.append({"name": city_name.capitalize(), "lat": lat, "lng": lon})

        final_response = {
            "route_points": route_for_frontend,
            "stops": stops_info,
...
def update_location():
    ...
    bus_data[bus_id] = {'id': data.get('bus_id'), 'number': data.get('number'), 'routeKey': data.get('routeKey'), 'latitude': float(data.get('latitude')), 'longitude': float(data.get('longitude')), 'status': data.get('status'), 'bearing': float(data.get('bearing')), 'last_updated': datetime.utcnow().isoformat() + 'Z'}
...
if __name__ == '__main__':
    print("Final Haryana Network Server with Caching is running on http://127.0.0.1:5000...")
